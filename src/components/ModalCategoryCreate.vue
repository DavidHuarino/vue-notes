        ...
        <div class="bg-black inset-0 bg-opacity-50 flex justify-center items-center absolute p-3 sm:w-3/5 sm:max-w-sm sm:m-auto">
            <div class="bg-gray-200 w-11/12 relative h-4/5 rounded-lg text-left">
                ...
                <section v-if="categories.length > 0" class="px-3 py-1 overflow-y-auto" :style="{'height': `calc(${windowHeight}px - 240px)`}">
                    <div v-for="(objectCategory, index) in categories" :key="index" class="my-2 p-1 border-b-2 border-blue-400 truncate cursor-pointer" @click="getSelectedCategoryName(objectCategory.name)">
                        {{objectCategory.name}}
                    </div>
                </section>
